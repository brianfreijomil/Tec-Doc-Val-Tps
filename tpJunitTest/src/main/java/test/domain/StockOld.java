...
import test.domain.Consumible;
import test.domain.Receta;
	...
	*/
	...
	public static boolean comprobarSuficientesIngredientes(Receta r) {
		...
		for(int i=0;i<r.getIngredientes().size(); i++) {
			Ingrediente iAux = r.getIngredientes().get(i).getIngrediente();
			completo = (iAux.getStock() > r.getIngredientes().get(i).getCantidad());
		}
		return completo;
	}
	...
	public static void consumirIngredientes(Receta r) {
		...
		for(int i=0;i<r.getIngredientes().size(); i++) {
			Consumible pAux = r.getIngredientes().get(i).getIngrediente();
